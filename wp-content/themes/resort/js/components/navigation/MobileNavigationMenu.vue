<template>
    <ul>
        <li v-for="(navitem, index) in mobileNavData" v-bind:key="index" class="nav-item" :class="{'dropdown': navitem.children.length > 0 }">
            <a v-if="navitem.url !='#' && navitem.url != ''" :href="navitem.url" :class="'nav-link'" :target="navitem.target" >{{ navitem.title }}</a>
            <span v-if="navitem.url =='#' || navitem.url == ''" @click="toggleSubMenu(index)" :class="'nav-link'" >{{ navitem.title }}</span>
            ...
            <div class="dropdown-menu text-center text-md-left" v-if="navitem.subMenuOpen" >
